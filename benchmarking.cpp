...
double sample_mean(const std::vector<double>& measurements) {
    // Computes the sample mean
    double sum = 0;
...
double sample_std(const std::vector<double>& measurements, double mean) {
    // Computes the corrected sample standard deviation
    double sum = 0;
...
std::vector<long> t_intersect(std::vector<std::vector<long>> sets, long t){
    std::multiset<long> multi_union {};
    for(auto & s : sets){
        multi_union.insert(s.begin(), s.end());
    }

    std::vector<long> res {};

    for(auto i : sets.at(sets.size()-1)){
        if(multi_union.count(i) >= t) res.push_back(i);
    }
    return res;
}

void ot_mpsi_benchmark(std::vector<long> parties_t, std::vector<long> set_size_exponents) {
    // Pre-generate all keys
    std::vector<std::pair<Keys, Keys>> keys;
    keys.reserve(parties_t.size());
    for (long t : parties_t) {
    ...
    for (int t_i = 0; t_i < parties_t.size(); ++t_i) {
        ...
        for (long exp : set_size_exponents) {
            ...
            for (int i = 0; i < 10; ++i) {
                auto start = std::chrono::high_resolution_clock::now();
                mpsi_to_over_threshold_multiparty_psi(experiment_sets.at(i), parties_t.at(t_i) / 2, m_bits, k_hashes,
                                         experiment_sets.at(0).size() / 2, keys.at(t_i).first);
                auto stop = std::chrono::high_resolution_clock::now();
            ...
            double mean1 = sample_mean(times);
            double std1 = sample_std(times, mean1);
            std::cout << "[(" << mean1 << ", " << std1  << "), ";
            times.clear();
            // threshold  t= t - 1
            ...
            double mean2 = sample_mean(times);
            double std2 = sample_std(times, mean2);
            std::cout << "(" << mean2 << ", " << std2 << ")], ";
        }
